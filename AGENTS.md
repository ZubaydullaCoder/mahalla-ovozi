...
* **Current Information:** For version-sensitive technologies, dependencies, APIs, or external services, verify decisions against current authoritative sources. Use efficiently one of best suited available tools: web search tool or context7 MCP server.
* **Project MCPs:** For Ant Design UI or Prisma database/schema work, use the available Ant Design and Prisma MCPs for relevant authoritative context and verification. Use Context7 MCP as the fallback for current package, API, framework, or tool documentation when a dedicated MCP is unavailable.
...
* **Architecture:** Like senior developer, follow KISS, YAGNI, DRY, Reusability, Modularity, Seperation of Concern, Maintainability, Scalability, Clean Code, SOLID, and other relevant best practices when applicable. Prefer cohesive, single-purpose domain logic and keep mutation at clear system boundaries.
