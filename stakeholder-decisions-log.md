...
| 2026-06-02 | Owner | Decision | AI / Filtering | Phase 1 should keep the existing full AI classification flow and add a developer/operator-side switch so `ai_full`, `keyword_gate`, and `shadow_compare` filtering approaches can be compared before choosing the pilot default. | Active | Treat filtering mode as a developer/operator configuration concern, not a hokim/staff dashboard feature. |
| 2026-06-02 | Owner | Decision | AI / Filtering | The client proposed a keyword-gated MVP flow: manually configured keywords pre-filter messages before AI, AI analyzes only keyword-matched messages, and non-keyword messages are skipped; the existing full-AI flow should remain available as a second approach. | Active | Support keyword-gated validation without deleting the full-AI path; evaluate both approaches with real/test data before locking the pilot mode. |
...
| 2026-06-01 | Owner | Decision | Product Scope | Tone classification is permanently removed from MVP scope: no tone field in AI output, no tone column in DB schema, no tone API field, no tone badge, and no tone filter. | Active | Do not reintroduce tone in any product, schema, API, UI, prompt, test, or story without explicit owner approval and PRD update. |
